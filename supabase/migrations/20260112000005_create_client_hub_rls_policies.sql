...
CREATE POLICY clients_select_policy ON clients
  FOR SELECT USING (
    assigned_to = auth.uid()::TEXT AND is_deleted = FALSE
  );

CREATE POLICY clients_insert_policy ON clients
  FOR INSERT WITH CHECK (
    assigned_to = auth.uid()::TEXT AND created_by = auth.uid()::TEXT
  );
...
CREATE POLICY client_tags_select_policy ON client_tags
  FOR SELECT USING (
    EXISTS (SELECT 1 FROM clients WHERE clients.id = client_tags.client_id AND clients.assigned_to = auth.uid()::TEXT AND clients.is_deleted = FALSE)
  );
...
CREATE POLICY client_tags_delete_policy ON client_tags
  FOR DELETE USING (
    EXISTS (SELECT 1 FROM clients WHERE clients.id = client_tags.client_id AND clients.assigned_to = auth.uid()::TEXT)
  );
...
CREATE POLICY client_documents_select_policy ON client_documents
  FOR SELECT USING (
    EXISTS (SELECT 1 FROM clients WHERE clients.id = client_documents.client_id AND clients.assigned_to = auth.uid()::TEXT AND clients.is_deleted = FALSE)
  );
...
CREATE POLICY client_documents_delete_policy ON client_documents
  FOR DELETE USING (
    EXISTS (SELECT 1 FROM clients WHERE clients.id = client_documents.client_id AND clients.assigned_to = auth.uid()::TEXT)
  );
...
CREATE POLICY client_notes_select_policy ON client_notes
  FOR SELECT USING (
    EXISTS (SELECT 1 FROM clients WHERE clients.id = client_notes.client_id AND clients.assigned_to = auth.uid()::TEXT AND clients.is_deleted = FALSE)
  );
...
CREATE POLICY client_notes_delete_policy ON client_notes
  FOR DELETE USING (
    EXISTS (SELECT 1 FROM clients WHERE clients.id = client_notes.client_id AND clients.assigned_to = auth.uid()::TEXT)
    AND created_by = auth.uid()::TEXT
  );
...
CREATE POLICY client_tasks_select_policy ON client_tasks
  FOR SELECT USING (
    ...
    EXISTS (SELECT 1 FROM clients WHERE clients.id = client_tasks.client_id AND clients.assigned_to = auth.uid()::TEXT AND clients.is_deleted = FALSE)
  );
...
CREATE POLICY client_tasks_delete_policy ON client_tasks
  FOR DELETE USING (
    EXISTS (SELECT 1 FROM clients WHERE clients.id = client_tasks.client_id AND clients.assigned_to = auth.uid()::TEXT)
    AND created_by = auth.uid()::TEXT
  );
